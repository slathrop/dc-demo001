...
Template.usersListPage.helpers({
  userInList: function() {
    ...
    if (PennySurveys.checkForHexCode.test(Session.get('usersListSearchFilter'))) {
      return Meteor.users.find({
        _id: new Meteor.Collection.ObjectID(Session.get('usersListSearchFilter'))
      });
    } else {
      return Meteor.users.find({
        $or: [{
            username: {
              $regex: Session.get('usersListSearchFilter'),
              $options: 'i'
            }
          }, {
            role: {
        ...
        limit: Session.get('usersListTableLimit'),
        skip: Session.get('usersListSkipCount')
      });
    }
  },
